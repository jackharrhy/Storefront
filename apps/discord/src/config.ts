...
export function readConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  const publicUrl = new URL(env.STOREFRONT_PUBLIC_URL || storefrontUrl);
  if (!["http:", "https:"].includes(publicUrl.protocol))
    throw new Error("STOREFRONT_PUBLIC_URL must be an HTTP or HTTPS URL.");
  ...
  return {
    token,
    commandPrefix,
    storefrontUrl: storefrontUrl.toString(),

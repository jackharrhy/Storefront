...
const { values } = parseArgs({ options: { output: { type: 'string' }, force: { type: 'boolean' } } });
const output = values.output ? resolve(values.output) : resolve(dirname(fileURLToPath(import.meta.url)), '../public/images');
...
async function main() {
  if (!values.force) {
    try {
      ...
      if (manifest.sha256 === sha256 && manifest.files?.length > 0 && manifest.files.every((name) => files.has(name))) {
  ...
  if (createHash('sha256').update(archive).digest('hex') !== sha256) throw new Error('Icon archive checksum mismatch');

  const prefix = `mc-assets-${revision}/item-assets/`;
  const icons = unzipSync(archive, {
    filter: ({ name }) => name.startsWith(prefix) && /^[A-Z0-9_]+\.png$/.test(name.slice(prefix.length)),
  });
  ...
  for (const [path, data] of Object.entries(icons)) {
    ...
    if (Buffer.from(data.subarray(0, 8)).toString('hex') !== '89504e470d0a1a0a') throw new Error(`Invalid PNG: ${name}`);
    await writeFile(join(output, `${name}.tmp`), data);
    await rename(join(output, `${name}.tmp`), join(output, name));
    files.push(name);
  }
  await writeFile(manifestPath, JSON.stringify({ source: url, revision, sha256, files: files.sort() }, null, 2) + '\n');
